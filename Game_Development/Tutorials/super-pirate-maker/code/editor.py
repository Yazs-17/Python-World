...
class Editor:
	...
	"""
	获取当前单元格（cell）坐标的方法，基于给定的对象或鼠标位置计算
		mouse_pos()：获取当前鼠标位置的函数
		self.origin：表示起点坐标的向量
		返回值是 col 和 row，分别表示所在单元格的列索引和行索引
	"""
	# support
	...
	"""
	检查单元格邻居的方法，基于给定的单元格坐标计算
		cell_pos：给定的单元格坐标
		self.canvas_data：表示画布数据的字典，包含单元格的属性信息，如是否有水或地形
		NEIGHBOR_DIRECTIONS：表示邻居方向的常量字典
	...
	# input
	def event_loop(self):
		# 进入编辑模式，当在界面中有任何操作(鼠标、键盘)都会进入该循环
		for event in pygame.event.get():
			if event.type == pygame.QUIT:
				pygame.quit()
				sys.exit()
			if event.type == pygame.KEYDOWN and event.key == pygame.K_RETURN:
				print('进入editor.event_loop函数中，并调用了switch')
